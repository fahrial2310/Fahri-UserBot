...
# untuk 
ALVIN_TEKS_CUSTOM = os.environ.get("ALVIN_TEKS_CUSTOM", None)
        ...
        pairs = pairs[
            ...
            (
                custom.Button.inline(
                    "◀️Back", data="{}_prev({})".format(prefix, modulo_page)
                ),
                custom.Button.inline(
...
with bot:
    try:
        ...
        LOGS.info(
            "your bot inline mode is disable. "
            "To Enable Go To @BotFather, then bot settings > select inline mode > Turn On. ")
    try:
        bot.loop.run_until_complete(check_botlog_chatid())
    except BaseException:
        LOGS.info(
