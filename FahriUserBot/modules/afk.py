...
# ========================= CONSTANTS ============================
AFKSTR = [
    f"**• sorry,my master {ALIVE_NAME} is afk!**",
    f"**• sorry,my master {ALIVE_NAME} is afk\n • Wait until he is back online!**",
    f"**• master {ALIVE_NAME} is afk!\n • Wait he is back online!**",
    f"**• sorry,my master {ALIVE_NAME} is afk!**",
]
...
async def type_afk_is_not_true(notafk):
    ...
    if ISAFK:
        ISAFK = False
        msg = await notafk.respond("**master is back *online* !!**")
        time.sleep(3)
        await msg.delete()
        ...
        if BOTLOG:
            await notafk.client.send_message(
                BOTLOG_CHATID,
                "master,you get " + str(COUNT_MSG) + " message from " +
                str(len(USERS)) + " Chat when you AFK",
            )
            for i in USERS:
    ...
    if mention.message.mentioned and not (await mention.get_sender()).bot:
        if ISAFK:
            ...
            if days == 1:
                ...
                afk_since = f"*{int(hours)} hour {int(minutes)} Minutes*"
            elif minutes > 0:
                afk_since = f"*{int(minutes)} Minutes {int(seconds)} second*"
            else:
                afk_since = f"*{int(seconds)} second*"
            if mention.sender_id not in USERS:
                if AFKREASON:
                    await mention.reply(f"- 𝗔 𝗙 𝗞 -\n `{ALIVE_NAME}` **afk** {afk_since} **Ago.**\
                        \n**• reason :** `{AFKREASON}`")
                else:
                ...
                if USERS[mention.sender_id] % randint(2, 4) == 0:
                    if AFKREASON:
                        await mention.reply(f"- 𝗔 𝗙 𝗞 -\n **master is still afk** {afk_since} **Ago.**\
                            \n**• reason :** `{AFKREASON}`")
                    else:
                        await mention.reply(str(choice(AFKSTR)))
