...
async def remove_all(event):
    ...
    await event.edit("_Delete a running download... _")
    await sleep(2.5)
    await event.edit("*succesfully delete all download*")
    await sleep(2.5)
...
async def pause_all(event):
    ...
    await event.edit("_pouse download...`")
    aria2.pause_all(force=True)
    await sleep(2.5)
    await event.edit("*succesfully poused running download*")
    await sleep(2.5)


@register(outgoing=True, pattern=r"^\.aresume(?: |$)(.*)")
async def resume_all(event):
    await event.edit("_resume download..._")
    aria2.resume_all()
    await sleep(1)
    await event.edit("*download resumed*")
    await sleep(2.5)
    await event.delete()
...
async def show_all(event):
    ...
    if len(msg) <= 4096:
        await event.edit("_Download in progress: _\n" + msg)
        await sleep(5)
        await event.delete()
    else:
        await event.edit("_Output is too large, sent as a file..._")
        output = "output.txt"
...
CMD_HELP.update(
    {
        "aria": ">;aurl [URL] (or) >;amag [link Magnet] (or) >;ator [jalur ke file torrent]"
        "\nUsage: Unduh file ke penyimpanan server bot pengguna Anda."
        "\n\n>;apause (or) ;aresume"
        "\nUsage: Pause/resume ongoing download."
        "\n\n>;aclear"
        "\nUsage: Clear download queue, delete all running downloads."
        "\n\n>;ashow"
        "\nUsage: Shows download progress in progress."})
