...
async def anime(event):
    ...
    if res:
        ...
        await event.edit("_No results Found!_")
        return
...
async def character(event):
    ...
    try:
        ...
        await event.edit("_Character not found._")
        return
...
async def whatanime(e):
    ...
    if not is_image(media) and not ig:
        await e.edit("_Media must be an image or gif or video_")
        return
    filename = "file.jpg"
    if not ig and isinstance(media, MessageMediaDocument):
        ...
        for i in attribs:
            if isinstance(i, DocumentAttributeFilename):
                ...
                break
    await e.edit("_Downloading image.._")
