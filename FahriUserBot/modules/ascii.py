...
async def ascii(event):
    if not event.reply_to_msg_id:
        await event.edit("_please reply to a media.._")
        return
    reply_message = await event.get_reply_message()
    if not reply_message.media:
        await event.edit("_reply to image/Sticker/Video_")
        return
    await event.edit("_downloading media.._")
    ...
    try:
        await event.edit("_in progress.._")
...
async def _(event):
    ...
    if BG.isnumeric():
        return await event.edit("_Please enter a color not a number master_")
    elif BG:
        global bground
        bground = BG
    else:
        return await event.edit("Please Enter Background From Ascii")
    await event.edit(f"*succesfully set a background from Ascii to* **{BG}**")


CMD_HELP.update(
    {
        "ascii": ";ascii\n"
        "Usage: Build Ascii Art from Media\n\n"
        ";asciis\n"
        "Usage: same but upload as sticker\n\n"
        ";asciibg <color>\n"
        "Usage: To Change The Background Color Of The Example Ascii Module _;asciibg black_"
    }
)
