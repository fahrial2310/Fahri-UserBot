...
async def log(log_text):
    ...
    if BOTLOG:
        if log_text.reply_to_msg_id:
            ...
            await log_text.edit("what am i Log?")
            return
        await log_text.edit("Logged succesfully!")
    else:
        await log_text.edit("This Feature Requires Login Enabled")
    await sleep(2)
...
async def kickme(leave):
    ...
    await leave.edit("bye,my master is not in this group,he leave ツ")
...
async def unmute_chat(unm_e):
    ...
    await unm_e.edit("*succesfully opened,chat is not muted*")
    await sleep(2)
    await unm_e.delete()
...
async def mute_chat(mute_e):
    ...
    await mute_e.edit("*Sssssttt master has muted this chat!*")
    await sleep(2)
    await mute_e.delete()
    if BOTLOG:
        await mute_e.client.send_message(
            BOTLOG_CHATID,
            str(mute_e.chat_id) + " has been muted.")
...
# Regex-Ninja module by @alvin_junior
regexNinja = False
...
async def sedNinjaToggle(event):
    """ enable or disable Module Regex Ninja. """
    global regexNinja
    if event.pattern_match.group(1) == "on":
        regexNinja = True
        await event.edit("*succesfully enable regex ninja module*")
        await sleep(1)
        await event.delete()
    elif event.pattern_match.group(1) == "off":
        regexNinja = False
        await event.edit("*succesfully disable regex ninja module*")
        await sleep(1)
        await event.delete()
...
async def info(event):
    await event.edit("_analisisi this chat..._")
    ...
    try:
        ...
        print("Exception:", e)
        await event.edit("_An Unexpected Error Occurred._")
    return
...
async def get_chatinfo(event):
    ...
    try:
        ...
        try:
            ...
            await event.edit("`Group/Channel invalid`")
            return None
        except ChannelPrivateError:
            await event.edit("_this group/channel is private or master is banned there_")
            return None
        except ChannelPublicGroupNaError:
            await event.edit("_channel or supergroup not found_")
            return None
...
async def fetch_info(chat, event):
    ...
    supergroup = "<b>Yes</b>" if hasattr(chat_obj_info,
                                         "megagroup") and chat_obj_info.megagroup else "not"
    slowmode = "<b>Yes</b>" if hasattr(
        chat_obj_info,
        "slowmode_enabled") and chat_obj_info.slowmode_enabled else "not"
    ...
    restricted = "<b>Yes</b>" if hasattr(chat_obj_info,
                                         "restricted") and chat_obj_info.restricted else "not"
    verified = "<b>Yes</b>" if hasattr(chat_obj_info,
                                       "verified") and chat_obj_info.verified else "not"
    ...
    if created is not None:
        caption += f"creator information: <code>{created.date().strftime('%b %d, %Y')} - {created.time()}</code>\n"
    else:
        caption += f"creator information: <code>{chat_obj_info.date.date().strftime('%b %d, %Y')} - {chat_obj_info.date.time()}</code> {warn_emoji}\n"
    caption += f"Data Centre ID: {dc_id}\n"
    if exp_count is not None:
        chat_level = int((1 + sqrt(1 + 7 * exp_count / 14)) / 2)
        caption += f"{chat_type} Level: <code>{chat_level}</code>\n"
    if messages_viewable is not None:
        caption += f"message can read: <code>{messages_viewable}</code>\n"
    if messages_sent:
        caption += f"sending message: <code>{messages_sent}</code>\n"
    elif messages_sent_alt:
        caption += f"sending message: <code>{messages_sent_alt}</code> {warn_emoji}\n"
    ...
    if members_online:
        caption += f"still Online: <code>{members_online}</code>\n"
    if restrcited_users is not None:
        caption += f"user on exception: <code>{restrcited_users}</code>\n"
    ...
    if hasattr(chat_obj_info, "exception"):
        caption += f"exception: {restricted}\n"
...
async def _(event):
    ...
    if event.is_private:
        await event.edit(";invite user to the chat, not in the private message")
    else:
        if not event.is_channel and event.is_group:
            ...
            await event.edit("_succesfully add user to the chat_")
        else:
...
\n\n;kickme\
\nPenjenexplanationlasan: leave from group.\
\n\n;link <username/userid>: <opsional teks> (or) reply to someone's message with ;link <teks opsional>\
\nexplanation: Create permalink to user profile with optional custom text.\
\n\n;regexninja enable/disabled\
\nexplanation: enable/disable modul ninja regex globally.\
\nModul help remove regex bot trigger message.\
\n\n;chatinfo [opsional: <reply/tag/chat/id/invite link>]`\
\nexplanation: get chat info. Some info may be restricted due to missing permissions..\
\n\n;invite \
\nexplanation: add user to the chat,not in private message. "
})

# Alvin Userbot
CMD_HELP.update({"mutechat": "**Modules:** __Mute Chat__\
\n\n**command:** ;mutechat\
\n**nexplanation:** mute all chat.\
\n\n**command:** ;unmutechat\
\n**nexplanation:** unlock muted chat."})
